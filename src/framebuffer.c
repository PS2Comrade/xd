...
/* it tracks whether current_fb points to client shm or our static buffer */
static int using_client_shm = 0;

void fb_init(void) { fb_clear(); }
...
/* check if are we looking at client shm right now */
int fb_is_client_buffer(void) { return using_client_shm; }
...
void fb_draw_rect(int x, int y, int w, int h, xd_color_t color) {
	/* four edges as lines int math avoids overflow surprises */
...
int fb_write_ppm(const char *path) {
	...
	/* ppm p6- binary rgb, no alpha support */
	fprintf(f, "P6\n%d %d\n255\n", XD_WIDTH, XD_HEIGHT);
