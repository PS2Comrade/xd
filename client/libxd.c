...
int xd_connect(void) {
    ...
    /* tell server about the new buffer */
    ...
    close(fd); /* server has its own reference now */
    return 0;
}
...
int xd_draw_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, xd_color_t color) {
    if (!shm_fb) return -1;
    xd_draw_line(x, y, x + w - 1, y, color);
    xd_draw_line(x, y + h - 1, x + w - 1, y + h - 1, color);
    xd_draw_line(x, y, x, y + h - 1, color);
    xd_draw_line(x + w - 1, y, x + w - 1, y + h - 1, color);
    return 0;
}
